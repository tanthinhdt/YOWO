...
from core.utils import read_truths_args
...
def data_augmentation(clip, shape, jitter, hue, saturation, exposure):
    ...
    # Augment
    cropped = [
        img.crop((pleft, ptop, pleft + swidth - 1, ptop + sheight - 1)) for img in clip
    ]
...
def load_data_detection(
    ...
    im_ind = int(im_split[num_parts - 1].split(".")[0])
    labpath = os.path.join(
        base_path, "labels", im_split[0], im_split[1], "{:05d}.txt".format(im_ind)
    )

    img_folder = os.path.join(base_path, "rgb-images", im_split[0], im_split[1])
    if dataset_use in ["ucf24", "vsl"]:
        max_num = len(os.listdir(img_folder))
    elif dataset_use in ["jhmdb21"]:
    ...
    if train:  # Apply augmentation
        ...
        try:
            tmp = torch.from_numpy(
                read_truths_args(labpath, 8.0 / clip[0].width).astype("float32")
            )
        except Exception:
            tmp = torch.zeros(1, 5)
...
def get_clip(root, imgpath, train_dur, num_samples):
    ...
    labpath = os.path.join(
        base_path, "labels", im_split[6], im_split[7], "{:05d}.txt".format(im_ind)
    )
